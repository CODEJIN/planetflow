...
- **행성 자전 보정(De-rotation)** — JPL Horizons 역표(astroquery) 기반; 고역통과 NCC 스윕으로 윈도우별 Warp Scale 자동 보정; 워프 전 디스크 중심 정렬로 워프 유도 편향 제거
...
| 04 | 자전 보정 스태킹 | 구면 워프 자전 보정 + 품질 가중 평균 스택; 고역통과 NCC Warp Scale 보정; 워프 전 디스크 중심 정렬 |
