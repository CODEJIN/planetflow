...
### 7.3 Warp Scale 자동 보정

De-rotation 워프 강도(warp scale)는 윈도우 내 가장 이른 프레임과 가장 늦은 프레임 사이에 고역통과 NCC 스윕을 수행하여 윈도우마다 자동으로 보정됩니다. 교차 상관 계산 전에 가우시안 고역통과 필터(σ=30 px)를 적용하여 림브 암화 편향을 제거하고, 벨트 구조가 최적 스케일 선택을 주도하도록 합니다. 보정된 값은 `derotation_log.json`의 `warp_scale` 필드에 기록됩니다 (목성 기준 통상 0.75–0.85). 사용자 입력이 필요하지 않습니다.
