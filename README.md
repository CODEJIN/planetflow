...
- **Planetary de-rotation** via JPL Horizons ephemeris (astroquery); warp-scale fixed at empirically-calibrated value (1.0); high-pass NCC sweep measures de-rotation confidence per session; warning dialog shown when NCC < 0.80; pre-warp disk-center alignment eliminates warp-induced alignment bias
...
| 04 | De-rotation Stack | Spherical-warp de-rotation + quality-weighted mean stack; warp-scale fixed at config value; high-pass NCC confidence measurement; warning when NCC < 0.80; pre-warp disk-center alignment |
