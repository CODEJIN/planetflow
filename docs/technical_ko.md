...
> **Warp Scale** (config 전용): 구면 워프 강도 배율 (`drift = warp_scale × Δλ_rad × depth(x,y)`)은 GUI에서는 고정 **0.80**으로 적용되며, `config.py → DerotationConfig.warp_scale`로만 변경 가능합니다. 이론값은 1.0이지만 대기 블러·플레이트 스케일 불확실성을 고려하면 목성 기준 0.80이 실험적 최적값입니다.
