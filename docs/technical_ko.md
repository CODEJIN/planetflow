...
NP.ang 조회 (번들 테이블 → 사용자 캐시 → 라이브 Horizons API)
    │
    ▼
자전 보정 신뢰도 측정 (고역통과 NCC, 고정 warp_scale: 최초 vs 최후 프레임, σ=30 px)
    │
    ▼
윈도우 내 각 프레임:
...
> **Warp Scale** (고정값): 구면 워프 강도 배율 (`drift = warp_scale × Δλ_rad × depth(x,y)`). 최적 시상 데이터에서 경험적으로 보정된 **물리 상수** — 목성 기본값 **1.0** (이론적 완전 구체 보정; 복수 데이터셋에서 NCC 피크가 1.0 부근으로 일관되게 확인됨). 행성의 자전 속도는 일정하므로 올바른 워프 기하 구조는 시상에 따라 변하지 않습니다. `config.py → DerotationConfig.warp_scale`에서 설정합니다.
...
**소스**: `pipeline/steps/derotate_stack.py` → `_measure_derot_confidence()`

Step 04 완료 후, 고역통과 NCC 스윕을 수행하여 자전 보정 예측이 실제 벨트 구조와 얼마나 잘 일치하는지를 정량화합니다. 이는 **진단 지표**로, 워프에 사용되는 `warp_scale`을 변경하지 않습니다.

**설계 근거**: `warp_scale`은 물리 상수(행성의 자전 기하 구조)입니다. 시상이 나빠도 올바른 워프 기하 구조는 변하지 않습니다 — 다만 벨트 구조가 흐릿해져 상관이 어려워질 뿐입니다. 기존 설계에서 warp_scale을 윈도우마다 자동 결정하려 했을 때, 나쁜 시상에서 보정 실패로 낮은 값(~0.5, 올바른 값 1.0 대신)이 반환되어 줄무늬 자전 보정 결과를 낳았습니다. NCC 결과는 이제 다른 질문에 답합니다: *"올바른 warp_scale을 적용했을 때, 이 세션에서 벨트 구조가 얼마나 보였는가?"*

**원본 NCC가 실패하는 이유**: 부드럽고 방사 대칭적인 림브 암화 그레디언트가 원본 NCC를 지배합니다. 어떤 워프도 이 그레디언트를 왜곡하므로 원본 NCC는 스케일이 커질수록 단조롭게 감소합니다.

**해결 — 가우시안 고역통과 필터**: NCC 계산 전 넓은 가우시안 블러(σ=30 px)를 빼서 림브 암화 성분을 제거합니다. 이렇게 하면 대기 벨트·존 구조만 남아 상관 계산을 주도합니다.
...
1. 세션에서 가장 긴 윈도우(전체 윈도우 중 최대 시간 스팬) 선택
2. 해당 윈도우의 최초 및 최후 TIF 프레임 로드
3. `scale`을 0.50 ~ 1.20 범위에서 13단계 + 설정값(1.0)을 명시적으로 포함하여 스윕
4. 각 스케일에서 `spherical_derotation_warp(frame_early, dt_total)`으로 순방향 예측 프레임 생성
5. 디스크 내부(r ≤ 0.7 × semi_a)에서 고역통과 NCC 계산
6. `ncc_at_config_scale` (warp_scale=1.0에서의 NCC)을 주 신뢰도 지표로 반환

**`derotation_summary.txt`에 기록되는 출력값**:

| 필드 | 의미 |
|---|---|
| `derot_confidence` | 설정된 warp_scale에서의 NCC — 주 신뢰도 지표 |
| `est. peak` | NCC 피크 스케일 — 진단용, 워프에는 사용되지 않음 |

**경고 임계값**: `ncc_at_config_scale < 0.80`이면 Step 04 완료 시(단독 실행) 또는 전체 단계 완료 후(일괄실행) GUI 경고 다이얼로그가 표시됩니다. 경고는 Step 3에서 윈도우를 줄이고 재실행하도록 권장합니다. 짧은 윈도우는 누적 회전량이 적어 나쁜 시상에서도 더 안정적입니다.
...
#### 그림자 위치 계산 알고리즘 (편평 타원체 레이 교점)

**소스**: `pipeline/modules/satellite_tracker.py` → `_shadow_pos_skyfield()`

프레임 타임스탬프 `t_obs`마다 아래 순서로 그림자 위치를 계산합니다:

1. **광행차시간(LTT) 보정**: `t_obs` 시점의 지구–목성 거리로부터 발광 시각 `t_emit = t_obs − d_EJ/c` (목성 기준 약 47분)를 역산합니다. 태양·목성·위성의 위치는 모두 `t_emit`에서 조회하여 Horizons 겉보기 위치 규약과 일치시킵니다.

2. **레이-타원체 교점 계산**: 태양에서 위성의 ICRF 위치를 통과하는 레이를 목성 편평 타원체(적도 반경 71 492 km, 극 반경 66 854 km, 극축 = IAU 2009 ICRF 방향)와 교차시킵니다. 지구 쪽 반구의 교점이 목성 표면의 그림자 위치입니다.

   ```
   레이: P(λ) = sun_km + λ × (moon_km − sun_km)
   유효 그림자: λ > 1  (태양 → 위성 → 목성 표면 순서)
   ```
...
**소스**: `pipeline/modules/satellite_tracker.py` → `detect_tracker_flip_ns()`

세션 설정에서 `tracker_flip_ns`가 자동(auto)으로 지정된 경우, 파이프라인은 벨트 밝기 비대칭을 분석하여 카메라가 북쪽 위(North-up)인지 남쪽 위(South-up)인지 추정합니다:

- 목성의 남적도대(SEB)는 북적도대(NEB)보다 일관되게 넓고 어둡습니다.
- 극 방향 각도를 기준으로 이미지를 회전한 뒤, 디스크 중심을 통과하는 수직 밝기 프로파일을 상·하 반부로 나눠 비교합니다.
- 종합 점수(`0.7 × 폭 비대칭 + 0.3 × 밝기 신호`)로 방향을 결정합니다. 신뢰도 임계값: 0.25.
- `flip_ns = True` → 남쪽 위 카메라(SEB가 이미지 상단). `flip_ns = False` → 북쪽 위 카메라.
- 신뢰도 < 0.25이면 결론 불가로 처리하고 사용자 지정값을 유지합니다.
