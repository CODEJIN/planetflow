...
| **Satellite Composite** | Off | — | Skyfield BSP 에페메리스를 사용해 유로파와 그림자를 합성합니다. 체크박스 옆 상태 표시기로 BSP 파일 가용 여부를 확인하세요 (§7.4 참고). |
...
| **Satellite Composite** | Off | — | 시계열 각 프레임에 유로파와 그림자를 합성합니다. 윈도우 ≥ 3 및 Skyfield BSP 파일 필요 (Step 04 §7.4와 동일한 상태 표시기). |
