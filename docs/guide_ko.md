...
| **윈도우 개수** | 1 | 1–20 | 탐지할 최적 윈도우의 수입니다. **1**: 가장 좋은 단일 윈도우만 찾습니다 (Step 04 스태킹용). **2~3**: 시간대별 여러 윈도우를 탐지합니다 (Step 08 시계열 합성용). |
...
| **Satellite Composite** | Off | — | Skyfield BSP 에페메리스를 사용해 유로파와 그림자를 합성합니다. 체크박스 옆 상태 표시기로 BSP 파일 가용 여부를 확인하세요 (§7.3 참고). |
